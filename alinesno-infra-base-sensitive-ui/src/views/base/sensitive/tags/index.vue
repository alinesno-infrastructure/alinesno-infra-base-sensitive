   ...
   <div class="app-container">
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="WordTagsList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="踢下线" placement="top">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:WordTags:edit']">踢下线</el-button>
                     </el-tooltip>
                     <el-tooltip content="强制注销" placement="top">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:WordTags:remove']">强制注销</el-button>
                     </el-tooltip>
                  </template>
               </el-table-column>
...
import {
   ...
   updateWordTags,
   addWordTags
} from "@/api/base/sensitive/word_tags";

const router = useRouter();
const { proxy } = getCurrentInstance();

// 定义变量
const WordTagsList = ref([]);
...
function getList() {
   loading.value = true;
   listWordTags(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
      WordTagsList.value = res.rows;
...
function handleDelete(row) {
   const WordTagsIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + WordTagsIds + '"的数据项？').then(function () {
      return delWordTags(WordTagsIds);
   }).then(() => {
      getList();
...
function handleUpdate(row) {
   reset();
   const WordTagsId = row.id || ids.value;
   getWordTags(WordTagsId).then(response => {
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.WordTagsId != undefined) {
            updateWordTags(form.value).then(response => {
               proxy.$modal.msgSuccess("修改成功");
               open.value = false;
               getList();
            });
         } else {
            addWordTags(form.value).then(response => {
